...
function ArenaRound({ match, portrait, meme, onProgress, onResolve, onClickSound, onTap }) {
	const [choices, setChoices] = (0, import_react.useState)(match.choices || {}), [phase, setPhase] = (0, import_react.useState)("choose"), [left, setLeft] = (0, import_react.useState)(7);
	const [shown, setShown] = (0, import_react.useState)({}), [selected, setSelected] = (0, import_react.useState)(null);
	const picks = (0, import_react.useRef)(match.choices || {}), end = (0, import_react.useRef)(match.roundDeadline || Date.now() + 7e3), closed = (0, import_react.useRef)(false);
	const origins = (0, import_react.useRef)({}), cards = (0, import_react.useRef)({}), jumps = (0, import_react.useRef)({});
	...
	const best = match.pair.reduce((a, b) => match.ratings[a] > match.ratings[b] ? a : b);
	const alive = match.players.map((p, i) => p.misses < 2 ? i : -1).filter((i) => i >= 0);
	function choose(player, id, event) {
		if (closed.current || Date.now() >= end.current || picks.current[player] !== void 0) return;
		const origin = origins.current[player]?.getBoundingClientRect(), target = cards.current[id]?.getBoundingClientRect();
		if (origin && target) jumps.current[player] = {
			x: origin.left - target.left - 8 - player * 22,
			y: origin.top - target.top + 16
		};
		picks.current = {
			...picks.current,
			[player]: id
		};
		setChoices(picks.current);
		callbacks.current.onProgress(picks.current, end.current);
		if (event) {
			setSelected(id);
			callbacks.current.onTap(event, id);
		}
	}
	(0, import_react.useEffect)(() => {
		callbacks.current.onProgress(picks.current, end.current);
		const bots = alive.filter((i) => i > 0 && picks.current[i] === void 0).map((i) => setTimeout(() => {
			choose(i, Math.random() < .56 ? best : match.pair.filter((x) => x !== best)[Math.floor(Math.random() * (match.pair.length - 1))]);
		}, 900 + Math.random() * 4100));
		let reveal, fall, done;
		const flights = [];
		const timer = setInterval(() => {
			const remaining = Math.max(0, end.current - Date.now());
			setLeft(remaining / 1e3);
			if (!closed.current && remaining === 0) {
				closed.current = true;
				clearInterval(timer);
				setPhase("portraits");
				alive.filter((i) => picks.current[i] !== void 0).map((i) => ({
					i,
					sort: Math.random()
				})).sort((a, b) => a.sort - b.sort).forEach(({ i }, index) => flights.push(setTimeout(() => {
					const id = picks.current[i], origin = origins.current[i]?.getBoundingClientRect(), target = cards.current[id]?.getBoundingClientRect();
					if (origin && target) jumps.current[i] = {
						x: origin.left - target.left - 8 - i * 22,
						y: origin.top - target.top + 16
					};
					setShown((current) => ({
						...current,
					...
					callbacks.current.onClickSound(1);
				}, index * 250 + Math.random() * 80)));
				reveal = setTimeout(() => setPhase("reveal"), 1e3);
				fall = setTimeout(() => setPhase("fall"), 1600);
				done = setTimeout(() => callbacks.current.onResolve(picks.current), 2500);
			}
		}, 35);
		return () => {
			bots.forEach(clearTimeout);
			flights.forEach(clearTimeout);
			clearInterval(timer);
			clearTimeout(reveal);
			clearTimeout(fall);
			clearTimeout(done);
	...
	function avatar(i, landed = false) {
		const jump = jumps.current[i] || {
			x: 0,
			y: -80
		};
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
			"aria-label": names[i],
			role: "img",
			className: "player-portrait portrait-" + i + (landed ? " portrait-jump" : ""),
			style: landed ? {
				"--jump-x": jump.x + "px",
				"--jump-y": jump.y + "px"
			} : void 0,
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "eyebrow lilac",
				children: [
					match.training ? "ТРЕНИРОВКА" : "АРЕНА · ДЕМО С БОТАМИ",
					" · РАУНД ",
					match.round
				]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "arena-players",
				children: match.players.map((p, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: p.misses >= 2 ? "out" : "",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "portrait-origin",
							ref: (el) => {
								origins.current[i] = el;
							},
							children: !shown[i] ? avatar(i) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "portrait-empty" })
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", { children: names[i] }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { children: p.misses >= 2 ? "Вылетел" : `${p.misses}/2 промаха` })
					]
				}, i))
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "timer",
				role: "timer",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", { style: { width: left / 7 * 100 + "%" } }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("b", { children: [left.toFixed(1), " с"] })]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
				className: "arena-status",
				role: "status",
				children: phase === "portraits" ? "Раскрываем выборы…" : phase !== "choose" ? "Верный вариант" : choices[0] !== void 0 ? "Выбор принят. Ждём конца таймера…" : match.players[0].misses >= 2 ? "Наблюдаем за выбором" : "Выбери более популярный мем"
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "cards arena-choice-cards " + (match.pair.length === 3 ? "three" : ""),
				children: match.pair.map((id) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					ref: (el) => {
						cards.current[id] = el;
					},
					className: "arena-choice " + (phase === "reveal" || phase === "fall" ? id === best ? "correct-card" : phase === "fall" ? "wrong-card" : "" : phase === "choose" && selected === id ? "own-choice" : ""),
					children: [
						meme(id, (event) => choose(0, id, event), phase !== "choose" || closed.current || choices[0] !== void 0 || match.players[0].misses >= 2),
						(phase === "reveal" || phase === "fall") && id === best && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "correct-label",
							children: "✓ Верно"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							className: "card-voters",
							children: match.players.map((_, i) => shown[i] && choices[i] === id ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								style: { left: 8 + i * 22 },
								children: avatar(i, true)
							}, i) : null)
						})
					]
				}, id))
...
const majorVersion = parseInt("19.2.6", 10);
...
* Documentation: [Base UI Dialog](https://base-ui.com/react/components/dialog)
*/
const DialogDescription$1 = /* @__PURE__ */ import_react.forwardRef(function DialogDescription(componentProps, forwardedRef) {
...
* Documentation: [Base UI Dialog](https://base-ui.com/react/components/dialog)
*/
const DialogPortal$1 = /* @__PURE__ */ import_react.forwardRef(function DialogPortal(props, forwardedRef) {
...
* Documentation: [Base UI Dialog](https://base-ui.com/react/components/dialog)
*/
const DialogTitle$1 = /* @__PURE__ */ import_react.forwardRef(function DialogTitle(componentProps, forwardedRef) {
	const { render, className, style, id: idProp, ...elementProps } = componentProps;
	const store = useDialogRootContext();
	const id = useBaseUiId(idProp);
...
* See the LICENSE file in the root directory of this source tree.
*/
const LucideContext = (0, import_react.createContext)({});
const useLucideContext = () => (0, import_react.useContext)(LucideContext);
//#endregion
...
* See the LICENSE file in the root directory of this source tree.
*/
const Icon = (0, import_react.forwardRef)(({ color, size, strokeWidth, absoluteStrokeWidth, className = "", children, iconNode, ...rest }, ref) => {
...
const fresh = () => ({
	...
	largeBrains: 0,
	votes: 0,
	ratings: Array.from({ length: 12 }, (_, i) => 1e3 + i * 37),
	owned: [],
	look: "",
	sound: true,
	match: null,
...
function migrate(s) {
	if (s?.version === 4) return {
		...s,
		largeBrains: s.largeBrains ?? 0
	};
	if (s?.version !== 3) return fresh();
	return {
		...fresh(),
		coins: s.coins,
		votes: s.votes,
...
function makePair(ratings, count, rng = Math.random) {
	const pool = ratings.map((r, id) => ({
		r,
		id
	}));
	...
	const chosen = [];
	for (const p of pool) {
		if (!chosen.some((x) => x.r === p.r)) chosen.push(p);
		if (chosen.length === count) return chosen.map((x) => x.id);
	}
	throw Error("Not enough distinct ratings");
}
function beginBatch(s, endless = false) {
	if (s.match) throw Error("Match active");
	if (s.batch) return s;
	const n = clone(s);
	n.batch = {
		endless,
		remaining: CONFIG.block,
		pair: makePair(n.ratings, 2),
		token: n.votes
	};
	return n;
}
function vote(s, id, pair, token) {
	if (s.match || !s.batch || token !== s.batch.token || JSON.stringify(pair) !== JSON.stringify(s.batch.pair) || pair.length !== 2 || !pair.includes(id)) throw Error("Invalid vote");
	const n = clone(s);
	n.ratings[id] += 7;
	n.ratings[pair.find((x) => x !== id)] -= 7;
	n.votes++;
	n.coins += CONFIG.voteReward;
	if (!n.batch.endless) n.batch.remaining--;
	if (n.batch.endless || n.batch.remaining) {
		n.batch.pair = makePair(n.ratings, 2);
		n.batch.token = n.votes;
	} else n.batch = null;
	return n;
}
function startMatch(s, training = false, rng = Math.random, mode = "brain", tierIndex = 0) {
	if (s.match) throw Error("Match active");
	const rules = MODES[mode], tier = rules?.tiers[tierIndex];
	if (!tier) throw Error("Invalid stake");
	if (!training && s[rules.currency] < tier.stake) throw Error("Insufficient balance");
	const n = clone(s);
	const entryFee = training ? 0 : tier.stake;
	n[rules.currency] -= entryFee;
	n.result = null;
	n.match = {
		id: Date.now(),
		mode,
		currency: rules.currency,
		maxRounds: tier.rounds,
		training,
		entryFee,
		prize: tier.coins,
		megaPrize: tier.mega,
		round: 1,
		ratings: [...s.ratings],
		players: Array.from({ length: mode === "brain" ? 1 : 3 }, () => ({ misses: 0 })),
		pair: makePair(s.ratings, 2, rng),
		finished: false
	};
	return n;
}
function round(s, answer, rng = Math.random, botAnswers) {
	if (!s.match || s.match.finished) throw Error("No active round");
	const n = clone(s), m = n.match;
	const best = m.pair.reduce((a, b) => m.ratings[a] > m.ratings[b] ? a : b);
	m.players.forEach((p, i) => {
		if (p.misses >= 2) return;
		if ((i === 0 ? answer : botAnswers ? botAnswers[i - 1] : rng() < .56 ? best : m.pair.find((x) => x !== best)) !== best) p.misses++;
	});
	const alive = m.players.map((p, i) => p.misses < 2 ? i : -1).filter((i) => i >= 0);
	if (m.mode === "brain") {
		if (m.players[0].misses >= 2) {
			m.finished = true;
			m.winner = -1;
		} else if (m.round >= m.maxRounds) {
			m.finished = true;
			m.winner = 0;
		} else {
			m.round++;
			m.pair = makePair(m.ratings, 2, rng);
		}
		return n;
	}
	if (alive.length <= 1) {
		m.finished = true;
		m.winner = alive.length === 1 ? alive[0] : null;
	} else {
		m.round++;
		m.pair = makePair(m.ratings, m.mode === "brain" ? 2 : m.round >= 3 ? 3 : 2, rng);
	}
	return n;
}
function settle(s) {
	if (!s.match?.finished) throw Error("Unfinished");
	const n = clone(s), m = n.match;
	const kind = m.winner === null ? m.players[0].misses >= 2 && m.players.some((p) => p.misses < 2) ? "lose" : "draw" : m.winner === 0 ? "win" : "lose";
	let coinsAward = 0, megaAward = 0, largeAward = 0;
	if (!m.training) {
		if (kind === "draw") if (m.currency === "megaCoins") megaAward = m.entryFee;
		else coinsAward = m.entryFee;
		if (kind === "win") {
			coinsAward = m.prize;
			megaAward = m.megaPrize;
			if (m.mode === "super") largeAward = 10;
		}
		n.coins += coinsAward;
		n.megaCoins += megaAward;
		n.largeBrains = (n.largeBrains ?? 0) + largeAward;
	}
	n.result = {
		kind,
		mode: m.mode || "super",
		currency: m.currency || "coins",
		training: m.training,
		coinsAward,
		megaAward,
		largeAward
...
function buy(s, id) {
	...
	const n = clone(s);
	if (!n.owned.includes(id)) {
		if (n.coins < item.price) throw Error("No coins");
		n.coins -= item.price;
		n.owned.push(id);
	}
	n.look = id;
...
function BrainCoin({ level = 0 }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
		className: "brain-coin",
		role: "img",
		"aria-label": [
			"Маленький мозг",
			"Средний мозг",
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "brain-icon level-" + level })
	});
}
function Cat({ look = "", small = false }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", {
		"aria-label": "Кот-волшебник",
		role: "img",
		className: small ? "cat small" : "cat",
		viewBox: "0 0 120 130",
		shapeRendering: "crispEdges",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
				fill: look === "skin" || look === "set" ? "#c298ef" : "#e9b77b",
				d: "M25 58h70v50H25zM19 49h19v25H19zM82 49h19v25H82zM34 107h19v15H34zM72 107h19v15H72zM88 94h22v9H88z"
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
				fill: "#ffe4af",
				d: "M39 85h45v23H39z"
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
				fill: "#152b25",
				d: "M34 72h12v12H34zM76 72h12v12H76zM55 88h11v6H55zM50 98h22v4H50z"
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
				fill: look === "hat" ? "#e0ce59" : look === "special" || look === "set" ? "#b785ee" : "#488bfa",
				d: "M20 53h85v11H20zM37 42h51v11H37zM44 27h36v15H44zM52 13h23v14H52zM61 0h14v13H61z"
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
				fill: "#ffe98a",
				d: "M59 31h8v8h-8zM71 14h5v5h-5z"
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
				fill: "#c98372",
				d: "M26 87h14v5H26zM82 87h14v5H82z"
			})
		]
	});
}
function Meme({ id, onClick, disabled, hit, handPose = {
	side: Date.now() % 2 ? "left" : "right",
	angle: Date.now() % 49 - 24
} }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
		className: "meme m" + id % 6,
		onClick,
		disabled,
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: "meme-caption",
				children: captions[id]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", {
				viewBox: "0 0 160 125",
				role: "img",
				"aria-label": captions[id],
				shapeRendering: "crispEdges",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
						fill: [
						...
						d: "M33 36h94v64H33zM25 20h25v38H25zM110 20h25v38h-25zM45 100h20v18H45zM95 100h20v18H95z"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
						fill: "#182a26",
						d: id % 2 ? "M44 61h24v6H44zM91 61h24v6H91zM67 82h30v7H67z" : "M48 54h14v20H48zM97 54h14v20H97zM74 81h13v15H74z"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
						fill: "#fff9db",
						d: "M49 54h5v6h-5zM98 54h5v6h-5z"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
						fill: "#e7957e",
						d: "M37 76h18v6H37zM108 76h18v6h-18z"
					}),
					id % 3 === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
						fill: "#728ade",
						d: "M21 16h119v12H21zM43 2h72v14H43z"
					}) : id % 3 === 1 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
						fill: "#67b2ed",
						d: "M112 73h8v25h-8zM119 91h7v12h-7z"
					}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
						fill: "#e6c142",
						d: "M38 27V6l21 14L80 0l19 20 22-14v21z"
					})
				]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
...
function Page() {
	const chestCollecting = (0, import_react.useRef)(false);
	const [adReward, setAdReward] = (0, import_react.useState)(1), [adWatching, setAdWatching] = (0, import_react.useState)(false);
	const adTimer = (0, import_react.useRef)(null), adClaim = (0, import_react.useRef)(false);
	(0, import_react.useEffect)(() => () => {
		if (adTimer.current) clearTimeout(adTimer.current);
	}, []);
	const [s, setS] = (0, import_react.useState)(null), [view, setView] = (0, import_react.useState)("home"), [pair, setPair] = (0, import_react.useState)([]), [hit, setHit] = (0, import_react.useState)(-1), [handPose, setHandPose] = (0, import_react.useState)({
		side: "right",
		angle: 0,
		key: 0,
		x: 0,
		y: 0,
		count: 1
	}), [likeBurst, setLikeBurst] = (0, import_react.useState)([]), [busy, setBusy] = (0, import_react.useState)(false), [modal, setModal] = (0, import_react.useState)(""), [reward, setReward] = (0, import_react.useState)(0), [msg, setMsg] = (0, import_react.useState)("Мемы сами себя не сварят."), [seconds, setSeconds] = (0, import_react.useState)(CONFIG.answerMs / 1e3), [ready, setReady] = (0, import_react.useState)(false), [tierIndex, setTierIndex] = (0, import_react.useState)(0);
	const ref = (0, import_react.useRef)(null), lock = (0, import_react.useRef)(false), deadline = (0, import_react.useRef)(0);
	function save(n) {
		ref.current = n;
		setS(n);
		try {
			localStorage.setItem("meme-cauldron-v4", JSON.stringify(n));
		} catch {
			setMsg("Браузер не смог сохранить прогресс.");
		}
	}
	(0, import_react.useEffect)(() => {
		let n = fresh();
		try {
			n = migrate(JSON.parse(localStorage.getItem("meme-cauldron-v4") || localStorage.getItem("meme-cauldron-v3") || "null"));
		} catch {}
		if (n.match?.finished) {
			n = settle(n);
			setView("result");
		} else if (n.match) setView("arena");
		else if (n.batch) setPair(n.batch.pair);
		save(n);
	}, []);
	function clickSound(count) {
		if (!ref.current.sound) return;
		try {
			const a = new AudioContext(), start = a.currentTime + .1;
			for (let tap = 0; tap < count; tap++) {
				const len = Math.floor(a.sampleRate * .075), buf = a.createBuffer(1, len, a.sampleRate), data = buf.getChannelData(0);
				for (let i = 0; i < len; i++) {
					const env = Math.pow(1 - i / len, 5);
					data[i] = (Math.random() * 2 - 1) * env * (i < 16 ? 1 : .45);
				}
				const src = a.createBufferSource(), filter = a.createBiquadFilter(), gain = a.createGain();
				filter.type = "bandpass";
				filter.frequency.value = 1750;
				filter.Q.value = 1.2;
				gain.gain.value = tap === 0 ? .28 : .24;
				src.buffer = buf;
				src.connect(filter);
				filter.connect(gain);
				gain.connect(a.destination);
				src.start(start + tap * .15);
				if (tap === count - 1) src.onended = () => a.close();
			}
		} catch {}
	}
	function happyReward() {
		if (!ref.current.sound) return;
		try {
			const a = new AudioContext(), start = a.currentTime + .12;
			const bell = a.createOscillator(), bg = a.createGain();
			bell.type = "sine";
			bell.frequency.setValueAtTime(1046, start);
			bell.frequency.setValueAtTime(1318, start + .09);
			bg.gain.setValueAtTime(.035, start);
			bg.gain.exponentialRampToValueAtTime(.001, start + .25);
			bell.connect(bg);
			bg.connect(a.destination);
			bell.start(start);
			bell.stop(start + .26);
			for (let i = 0; i < 3; i++) {
				const t = start + .28 + i * .17, o = a.createOscillator(), filter = a.createBiquadFilter(), g = a.createGain();
				o.type = "sawtooth";
				o.frequency.setValueAtTime(440, t);
				o.frequency.linearRampToValueAtTime(680, t + .035);
				o.frequency.exponentialRampToValueAtTime(350, t + .145);
				filter.type = "bandpass";
				filter.Q.value = 2;
				filter.frequency.setValueAtTime(1200, t);
				filter.frequency.exponentialRampToValueAtTime(550, t + .15);
				g.gain.setValueAtTime(.001, t);
				g.gain.linearRampToValueAtTime(.045, t + .025);
	...
	function triggerHand(event, single = false) {
		const box = event.currentTarget.getBoundingClientRect(), count = single ? 1 : Math.random() < .5 ? 1 : 2;
		const x = event.detail === 0 ? box.left + box.width / 2 : event.clientX, y = event.detail === 0 ? box.top + box.height / 2 : event.clientY;
		setHandPose({
			side: Math.random() < .5 ? "left" : "right",
			angle: Math.round(-18 + Math.random() * 36),
			key: Date.now(),
	...
	function startBrew(event) {
		lock.current = true;
		setBusy(true);
		triggerHand(event, true);
		setHit(99);
		save(beginBatch(ref.current, modal === "mode"));
		setModal("");
		setTimeout(() => {
	...
	function brew(event) {
		if (busy || lock.current || ref.current.batch) return;
		setModal("mode");
	}
	function chestVictory() {
		happyReward();
		if (!ref.current?.sound) return;
		try {
			...
			[
				...
				const o = a.createOscillator(), g = a.createGain(), t = a.currentTime + i * .12;
				o.type = "triangle";
	...
	async function collectChest(event) {
		if (chestCollecting.current || !ref.current.chestPending) return;
		chestCollecting.current = true;
		const el = event.currentTarget, box = el.getBoundingClientRect(), target = document.querySelector(".inventory-button")?.getBoundingClientRect();
		if (target) {
			const dx = target.left + target.width / 2 - box.left - box.width / 2, dy = target.top + target.height / 2 - box.top - box.height / 2;
			try {
				await el.animate([{
					transform: "translate(0,0) rotate(0deg) scale(1)",
					opacity: 1
				}, {
					transform: `translate(${dx}px,${dy}px) rotate(720deg) scale(.12)`,
					opacity: .3
				}], {
					duration: 850,
					easing: "cubic-bezier(.45,0,.8,.6)",
					fill: "forwards"
				}).finished;
			} catch {}
		}
		save({
			...ref.current,
			chestPending: false,
			chests: (ref.current.chests || 0) + 1
		});
		chestCollecting.current = false;
	}
	function grumpyMeow() {
		if (!ref.current?.sound) return;
		try {
			const a = new AudioContext(), o = a.createOscillator(), g = a.createGain(), f = a.createBiquadFilter(), t = a.currentTime;
			o.type = "sawtooth";
			o.frequency.setValueAtTime(330, t);
			o.frequency.linearRampToValueAtTime(220, t + .16);
			o.frequency.linearRampToValueAtTime(145, t + .48);
			f.type = "bandpass";
	...
	function watchAd() {
		if (adClaim.current) return;
		adClaim.current = true;
		setAdWatching(true);
		adTimer.current = setTimeout(() => {
			save({
				...ref.current,
				megaCoins: ref.current.megaCoins + adReward
			});
			setAdWatching(false);
			setModal("");
			adClaim.current = false;
			happyReward();
		}, 5e3);
	}
	function closeBatch() {
	...
	function choose(id, event) {
		if (modal || lock.current || !ref.current.batch) return;
		lock.current = true;
		triggerHand(event);
		setHit(id);
		setLikeBurst(Array.from({ length: 2 + Math.floor(Math.random() * 2) }, (_, i) => ({
			id: Date.now() + i,
			x: event.clientX,
			y: event.clientY,
			dx: (i % 2 === 0 ? -1 : 1) * (80 + Math.random() * 100),
			dy: -(50 + Math.random() * 70),
			rot: (Math.random() - .5) * 140,
			scale: 1 + Math.random() * .8
		})));
		setTimeout(() => setLikeBurst([]), 700);
		const n = vote(ref.current, id, pair, ref.current.batch.token);
		save(n);
		setReward(n.votes);
		happyReward();
		...
		setTimeout(() => {
			...
			if (dropChest) {
				...
				chestVictory();
			} else if (offerAd && ref.current.batch) {
				setAdReward(1 + Math.floor(Math.random() * 5));
				setModal("ad");
				grumpyMeow();
			}
		}, 820);
		setTimeout(() => setReward((current) => current === n.votes ? 0 : current), 1100);
	}
	function enter(training = false) {
		if (lock.current) return;
		try {
			save(startMatch(ref.current, training, Math.random, view === "super" ? "super" : "brain", tierIndex));
			setView("arena");
			setReady(false);
		} catch {
			setMsg("Недостаточно монет для этой ставки.");
		}
	}
	function answer(id) {
		if (lock.current || !ref.current.match || ref.current.match.finished) return;
		lock.current = true;
		setReady(false);
		setHit(id ?? -1);
		const delay = ref.current.match.mode === "brain" && id !== null ? 820 : 400;
		setTimeout(() => {
			const n = round(ref.current, id);
			setHit(-1);
			if (n.match.finished) {
				save(settle(n));
				setView("result");
	...
	const roundKey = s?.match ? `${s.match.id}-${s.match.round}` : "";
	(0, import_react.useEffect)(() => {
		if (view !== "arena" || !s?.match || s.match.finished || s.match.mode !== "brain") return;
		let canceled = false;
		setReady(false);
		setSeconds(CONFIG.answerMs / 1e3);
		const t = setTimeout(() => requestAnimationFrame(() => requestAnimationFrame(() => {
			if (canceled) return;
			deadline.current = performance.now() + CONFIG.answerMs;
			setReady(true);
		})), 450);
		return () => {
			canceled = true;
	...
	const m = s?.match;
	const nav = (v) => {
		if (!m && !busy && !lock.current) {
			setView(v);
			setTierIndex(0);
			if (v === "home") setPair(ref.current.batch?.pair || []);
		}
	};
	if (!s) return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("main", {
		className: "phone",
		children: [likeBurst.map((l) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
			className: "like-fly",
			style: {
				left: l.x,
				top: l.y,
			...
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("img", {
				src: "/ADHD-Bro/assets/like-thumb.png",
				alt: ""
			})
		}, l.id)), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "Разогреваем котёл…" })]
	});
	const mode = view === "super" ? "super" : "brain", rules = MODES[mode], tier = rules.tiers[tierIndex], unit = mode === "super" ? "средних мозгов" : "маленьких мозгов";
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("main", {
		className: "phone",
		children: [
			likeBurst.map((l) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: "like-fly",
				style: {
					left: l.x,
					top: l.y,
			...
			hit >= 0 && (0, import_react_dom.createPortal)(/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "hand-impact",
				style: {
					left: handPose.x,
					top: handPose.y,
					"--hand-x": handPose.side === "left" ? "-120px" : "120px",
					"--hand-animation": handPose.count === 2 ? "knock-double" : "knock-single"
				},
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("img", {
					className: "tap-hand",
					src: "/ADHD-Bro/assets/hand.png",
					alt: "",
					style: { transform: `rotate(${handPose.angle}deg) scaleX(${handPose.side === "left" ? -1 : 1})` }
				})
			}, handPose.key), document.body),
			view === "home" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: reward ? "scene cat-happy" : "scene",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							...
							disabled: busy || hit >= 0,
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("img", {
								src: "/ADHD-Bro/assets/inventory-bag.png",
								alt: ""
							})
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Cat, { look: s.look }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							className: "cauldron",
							onClick: brew,
							disabled: busy || !!s.batch,
							"aria-label": "Постучать по котлу и получить мемы",
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", {
								...
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
										fill: "#f2793e",
										d: "M60 120h25v29h22v-19h20v-15h18v32h20v-28h16v38H60z"
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
										fill: "#ffcc52",
										d: "M87 132h15v25h38v-23h15v31H87z"
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
										fill: s.look === "set" ? "#654b85" : "#274a43",
										d: "M32 37h176v66h-12v22H45v-22H32z"
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
										fill: "#101e1b",
										d: "M17 30h206v16H17zM22 47h17v26H22zM201 47h17v26h-17zM46 120h24v18H46zM171 120h24v18h-24z"
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
										fill: "#b8f854",
										d: "M37 22h166v17H37zM55 10h18v18H55zM148 0h12v26h-12zM178 11h17v18h-17z"
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
										fill: "#72b742",
										d: "M54 43h134v10H54z"
									...
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
										fill: "#9cff5b",
										d: "M91 77h58v9H91zM107 65h26v34h-26z"
									})
								]
							})
						})
				...
				pair.length > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "cards home-cards",
					children: [s.batch?.endless && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
						className: "endless-mark",
						viewBox: "0 0 32 16",
						role: "img",
						"aria-label": "Бесконечные мемы",
						shapeRendering: "crispEdges",
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
							...
							d: "M16 8 10 3H4L1 6v4l3 3h6L22 3h6l3 3v4l-3 3h-6Z"
						})
					}), pair.map((id) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Meme, {
						id,
						handPose,
						hit: hit === id,
				...
				!s.batch && s.votes > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					...
					children: "Снова варим?"
				})
			] }),
			(view === "brain" || view === "super") && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
				className: "mode-lobby",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "brain-icon hero-brain level-" + (mode === "brain" ? 1 : 2),
						"aria-hidden": "true"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "eyebrow lilac",
						children: mode === "brain" ? "СОЛО · ДВА МЕМА" : "АРЕНА · ДВА БОТА"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { children: [
						mode === "brain" ? "Пройди все раунды. Два промаха — вылет." : "Останься последним. Два промаха — вылет.",
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						...
						children: rules.tiers.map((t, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
							"aria-pressed": tierIndex === i,
							className: tierIndex === i ? "selected" : "",
							onClick: () => setTierIndex(i),
							children: [
								t.stake,
								" ",
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)(BrainCoin, { level: mode === "brain" ? 0 : 1 }),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { children: t.rounds ? t.rounds + " раундов" : "×3 за победу" })
							]
						}, t.stake))
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "panel",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: "Награда за победу" }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "prize",
								children: [
									tier.coins > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
										tier.coins,
										" ",
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)(BrainCoin, {}),
										" + "
									...
									tier.mega,
									" ",
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)(BrainCoin, { level: 1 }),
									mode === "super" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("small", {
										className: "large-prize",
										children: [" + 10 ", /* @__PURE__ */ (0, import_jsx_runtime.jsx)(BrainCoin, { level: 2 })]
									})
								]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
								className: "fine",
								children: [mode === "brain" ? "Двойная ставка и средние мозги." : "Тройная ставка средними мозгами.", " Ставка списывается при входе."]
							})
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
						disabled: s[rules.currency] < tier.stake,
					...
					s[rules.currency] < tier.stake && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
						className: "notice",
						children: [
							"Не хватает ",
							tier.stake - s[rules.currency],
							" ",
							unit,
							". ",
							mode === "brain" ? "Заработай маленькие мозги в котле." : "Побеждай в соло-режиме."
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
						className: "action quiet",
						onClick: () => enter(true),
						children: "Тренировка · бесплатно"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
						className: "fine",
						children: ["Без наград в тренировке.", mode === "super" ? " При общей ничьей ставка возвращается." : ""]
					})
				]
			}),
			view === "arena" && m && m.mode === "brain" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "eyebrow lilac",
					children: [
						m.training ? "ТРЕНИРОВКА" : m.mode === "brain" ? "СОЛО" : "АРЕНА · БОТЫ",
						" • РАУНД ",
						m.round,
				...
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: m.mode === "brain" ? "players solo-player" : "players",
					children: m.players.map((p, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: p.misses >= 2 ? "out" : "",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", { children: [
							"Ты",
							"Бот: Кусь",
							"Бот: Жмых"
						][i] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: p.misses >= 2 ? "Вылетел" : `${p.misses}/2 промаха` })]
					}, i))
				}),
				m.mode === "brain" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "scene solo-scene",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "star one",
							children: "✦"
				...
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", {
					className: m.mode === "brain" ? "sr-only" : "",
					children: m.players[0].misses >= 2 ? m.mode === "brain" ? "Два промаха" : "Кот наблюдает" : "Лови мем!"
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "timer",
					role: "timer",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", { style: { width: `${seconds / (CONFIG.answerMs / 1e3) * 100}%` } }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("b", { children: [seconds.toFixed(1), " с"] })]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "cards " + (m.mode === "brain" ? "home-cards " : m.pair.length === 3 ? "three" : ""),
					children: m.pair.map((id) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Meme, {
						id,
						handPose,
						hit: hit === id,
						onClick: (event) => {
							if (performance.now() < deadline.current) triggerHand(event);
							answer(performance.now() < deadline.current ? id : null);
						},
						disabled: !ready || lock.current || m.players[0].misses >= 2
					}, id))
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "fine",
					children: m.players[0].misses >= 2 ? m.mode === "brain" ? "Попробуем ещё?" : "Боты заканчивают замес…" : "Выбери более популярный мем."
				})
			] }),
			view === "arena" && m && m.mode !== "brain" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ArenaRound, {
				match: m,
				portrait: (i) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Cat, {
					small: true,
					look: i === 0 ? s.look : i === 1 ? "special" : "skin"
				}),
				meme: (id, onClick, disabled) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Meme, {
					id,
					onClick,
					disabled
				}),
				...
				onTap: (event, id) => {
					triggerHand(event, true);
					setHit(id);
					setTimeout(() => setHit(-1), 400);
				},
				onProgress: (choices, roundDeadline) => {
					if (ref.current.match) save({
						...ref.current,
						match: {
							...ref.current.match,
							choices,
				...
				onResolve: (choices) => {
					if (!ref.current.match || ref.current.match.finished) return;
					const n = round(ref.current, choices[0] ?? null, Math.random, [choices[1] ?? null, choices[2] ?? null]);
					delete n.match.choices;
					delete n.match.roundDeadline;
					if (n.match.finished) {
						save(settle(n));
						setView("result");
					} else save(n);
			...
			view === "result" && s.result && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "eyebrow",
					children: s.result.kind === "win" ? "ПОБЕДА" : s.result.kind === "draw" ? "НИЧЬЯ" : "ПОРАЖЕНИЕ"
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Cat, { look: s.look }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", { children: s.result.kind === "win" ? "Легенда котла!" : s.result.kind === "draw" ? "Все в супе." : "Кот.exe завис" }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: s.result.kind === "win" ? "Мяу. Это было мощно." : s.result.kind === "draw" ? "Ничья. Кот никого не осуждает." : "Сегодня мемы были сильнее." }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "result-pay",
					children: s.result.training ? "Тренировка · без наград" : s.result.kind === "lose" ? "Без награды" : [
						[s.result.coinsAward, "маленьких мозгов"],
						[s.result.megaAward, "средних мозгов"],
						[s.result.largeAward ?? 0, "больших мозгов"]
					].filter(([amount]) => Number(amount) > 0).map(([amount, label]) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
						className: "result-resource",
						children: [
							"+",
							amount,
							" ",
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(BrainCoin, { level: label === "маленьких мозгов" ? 0 : label === "средних мозгов" ? 1 : 2 }),
							s.result.kind === "draw" ? " · возврат" : ""
						]
					}, String(label)))
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
					className: "action purple",
					onClick: () => nav(s.result.mode || "super"),
					children: "Ещё замес"
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
					className: "action",
					onClick: () => nav("wardrobe"),
					children: "Примерить величие"
				})
			] }),
			view === "wardrobe" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "eyebrow",
					children: "ГАРДЕРОБ"
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", { children: "Кот на стиле" }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Cat, { look: s.look }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
					className: "action quiet",
					onClick: () => save({
				...
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					...
					children: ITEMS.map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "shop-item",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Cat, {
								small: true,
								look: item.id
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: item.name }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
								className: "coins",
								children: [
									item.price,
									" ",
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
								disabled: s.look === item.id || !s.owned.includes(item.id) && s.coins < item.price,
								onClick: () => {
									try {
										save(buy(ref.current, item.id));
									} catch {}
								},
								children: s.look === item.id ? "Надето" : s.owned.includes(item.id) ? "Надеть" : "Купить"
							})
						]
					}, item.id))
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "fine",
					children: "Купленное остаётся твоим. Переодеваться бесплатно."
				})
			] }),
			view === "wardrobe" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
				...
				children: [
					...
					disabled: !!m,
					"aria-current": view === v || view === "arena" && m?.mode === v ? "page" : void 0,
					className: view === v || view === "arena" && m?.mode === v ? "active" : "",
					onClick: () => nav(v),
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							...
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								"aria-hidden": "true",
								className: "brain-icon level-" + icon
							})
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "sr-only",
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", {
							...
							children: icon === "0" ? s.coins : icon === "1" ? s.megaCoins : s.largeBrains ?? 0
						})
					]
				}, v))
			}),
			view === "home" && s.chestPending && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
				className: "treasure-drop",
				"aria-label": "Забрать сундук в инвентарь",
				onClick: collectChest,
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("img", {
					src: "/ADHD-Bro/assets/treasure-chest.png",
					alt: ""
				})
			}),
			view === "wardrobe" && (s.chests || 0) > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "inventory-chests",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("img", {
					src: "/ADHD-Bro/assets/treasure-chest.png",
					alt: "Сундуки"
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("b", { children: ["× ", s.chests] })]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Dialog, {
				open: modal === "ad",
				onOpenChange: (open) => {
					if (!open && !adWatching) setModal("");
				},
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(DialogContent, {
					className: "game-dialog ad-dialog",
					showCloseButton: false,
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(DialogTitle, {
							className: "sr-only",
							children: "Добровольный просмотр рекламы"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(DialogDescription, {
							className: "sr-only",
							children: [
								"Тестовый просмотр длится пять секунд. Награда: ",
								adReward,
								" средних мозгов. Можно отказаться без штрафа."
							]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", {
							className: "ad-play",
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
								fill: "#f03939",
								d: "M4 2h24v2h2v16h-2v2H4v-2H2V4h2z"
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
								fill: "white",
								d: "M12 6h3v2h3v2h3v4h-3v2h-3v2h-3z"
							})]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "ad-prize",
						...
						adWatching ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							className: "ad-progress",
							role: "progressbar",
							"aria-label": "Тестовый просмотр рекламы",
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", {})
						}) : /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "ad-choices",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								"aria-label": "Смотреть рекламу",
								onClick: watchAd,
								children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
									...
									children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
										fill: "#83ef54",
										d: "M2 11h4v4h4v-4h4V7h4V3h4v8h-4v4h-4v4h-4v4H6v-4H2z"
									})
								})
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								"aria-label": "Отказаться от рекламы",
								onClick: () => setModal(""),
								children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
									viewBox: "0 0 24 24",
									shapeRendering: "crispEdges",
									children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
										fill: "#ff4848",
										d: "M2 2h5v5h5v5H7v5H2v5h5v-5h5v-5h5v5h5v5h-5v-5h-5v-5h5V7h5V2h-5v5h-5v5H7V7H2z"
									})
								})
							})]
						})
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Dialog, {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(DialogContent, {
					...
					showCloseButton: false,
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(DialogTitle, { children: "Как варим мемы?" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(DialogDescription, { children: "Выбери способ набора карточек" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "mode-choice-grid",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
								className: "action",
								onClick: (e) => startBrew(e),
								children: ["↻", /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { children: "Случайные мемы" })]
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
								className: "action",
								onClick: () => setModal("categories"),
								children: ["▦", /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { children: "Выбрать категорию" })]
							})]
						})
					]
				})
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Dialog, {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(DialogContent, {
					className: "game-dialog category-dialog",
					showCloseButton: false,
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(DialogTitle, {
							className: "sr-only",
							children: "Категории мемов"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							...
							children: MEME_CATEGORIES.map(([name, icon]) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
								"aria-label": name,
								className: "category-tile",
								onClick: (e) => startBrew(e),
								children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: icon })
							}, name))
						})
					]
				})
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Dialog, {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(DialogContent, {
					...
					showCloseButton: false,
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(DialogTitle, { children: "Начать с чистого котла?" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(DialogDescription, { children: "Маленькие, средние и большие мозги и внешки в этом браузере будут удалены." }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
							className: "action",
							onClick: () => {
								save(fresh());
							...
							children: "Да, сбросить прогресс"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
							className: "action quiet",
							onClick: () => setModal(""),
							children: "Закрыть без сброса"
						})
					]
				})
			}),
			s.batch && view === "home" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
				className: "batch-close",
				onClick: closeBatch,
				"aria-label": "Закрыть серию мемов",
				children: "×"
			})
		]
	});
...
//#endregion
//#region app/DancingFish.tsx
function DancingFish() {
	const canvas = (0, import_react.useRef)(null);
	(0, import_react.useEffect)(() => {
		let stopped = false, timer;
		const image = new Image();
		image.src = "/ADHD-Bro/assets/dancing-fish-sequence-v3.png";
		image.onload = () => {
			if (stopped) return;
			...
			const pixels = source.getImageData(0, 0, sheet.width, sheet.height);
			for (let i = 0; i < pixels.data.length; i += 4) {
				const r = pixels.data[i], g = pixels.data[i + 1], b = pixels.data[i + 2];
				if (Math.min(r, g, b) > 195 && Math.max(r, g, b) - Math.min(r, g, b) < 25) pixels.data[i + 3] = 0;
			}
			source.putImageData(pixels, 0, 0);
			const ctx = canvas.current.getContext("2d");
			ctx.imageSmoothingEnabled = false;
			const pose = document.createElement("canvas");
			pose.width = pose.height = 314;
			const poseContext = pose.getContext("2d");
			poseContext.imageSmoothingEnabled = false;
			const dance = [
				0,
				1,
				2,
			...
			let frame = 0;
			const draw = () => {
				const turning = frame >= 16, cell = turning ? 14 : dance[frame];
				const x = Math.round(cell % 4 * sheet.width / 4), y = Math.round(Math.floor(cell / 4) * sheet.height / 4), right = Math.round((cell % 4 + 1) * sheet.width / 4), bottom = Math.round((Math.floor(cell / 4) + 1) * sheet.height / 4);
				poseContext.clearRect(0, 0, 314, 314);
				poseContext.drawImage(sheet, x, y, right - x, bottom - y, 0, 0, 314, 314);
				ctx.clearRect(0, 0, 314, 314);
				if (!turning) ctx.drawImage(pose, 0, 0);
				else {
					const angle = (frame - 16) / 16 * Math.PI * 2, cos = Math.cos(angle), sin = Math.sin(angle);
					const order = sin >= 0 ? [1, 0] : [0, 1];
					for (const side of order) {
						const scale = 1 + (side === 0 ? 1 : -1) * sin * .13;
						const horizontal = (cos < 0 ? -1 : 1) * (.5 + .5 * Math.abs(cos));
						ctx.save();
						ctx.translate(157, 130);
						ctx.scale(horizontal * scale, scale);
						ctx.drawImage(pose, side * 157, 0, 157, 314, side === 0 ? -157 : 0, -130, 157, 314);
						ctx.restore();
					}
				}
				frame = (frame + 1) % 32;
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("canvas", {
		...
		width: 314,
		height: 314,
		className: "dancing-fish-sprite",
		role: "img",
		"aria-label": "Две танцующие пиксельные рыбки"
	});
}
//#endregion
//#region app/chiptune3.ts
function startChiptune() {
	...
	let i = 0;
	const n = (type, f, t, d, v) => {
		const o = c.createOscillator(), g = c.createGain();
		o.type = type ? "sawtooth" : "square";
		o.frequency.value = f;
		g.gain.setValueAtTime(v, t);
		g.gain.exponentialRampToValueAtTime(.006, t + d);
		o.connect(g);
		g.connect(m);
		o.start(t);
		o.stop(t + d + .01);
	...
	const beat = () => {
		const t = c.currentTime + .02, k = i % 32;
		n(0, hook[k], t, .11, .42);
		if (i % 2 === 0) n(1, bass[Math.floor(i / 2) % 16], t, .2, .22);
		if (i % 4 === 0) n(1, bass[Math.floor(i / 2) % 16] / 2, t, .32, .25);
		if (i % 8 === 4) n(0, hook[(k + 2) % 32] * 2, t, .05, .12);
		i++;
	};
	beat();
	const id = setInterval(beat, 105);
	return () => {
		clearInterval(id);
		c.close();
	};
...
function StarTunnel() {
	...
	(0, import_react.useEffect)(() => {
		...
		function resize() {
			w = el.width = Math.ceil(innerWidth / 3);
			h = el.height = Math.ceil(innerHeight / 3);
		}
		resize();
		...
		function draw() {
			...
			const cx = w / 2, cy = h * .42, spread = Math.max(w, h) * .22;
			for (const s of stars) {
				...
				const radius = spread * s.r / s.z, previous = spread * s.r / (s.z + .045), x = cx + Math.cos(s.a) * radius, y = cy + Math.sin(s.a) * radius;
				ctx.strokeStyle = `rgba(117,216,255,${Math.min(1, .18 + (1 - s.z) * .9)})`;
				ctx.lineWidth = s.z < .2 ? 1.1 : .65;
				ctx.beginPath();
				ctx.moveTo(Math.round(cx + Math.cos(s.a) * previous), Math.round(cy + Math.sin(s.a) * previous));
				ctx.lineTo(Math.round(x), Math.round(y));
				ctx.stroke();
			}
		}
		draw();
		const tick = setInterval(draw, 125);
		return () => {
			clearInterval(tick);
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("canvas", {
		className: "star-tunnel",
		ref: canvas,
		"aria-hidden": "true"
	});
}
function LoadingScreen() {
	const [frame, setFrame] = (0, import_react.useState)(0), [ready, setReady] = (0, import_react.useState)(false), [opening, setOpening] = (0, import_react.useState)(false), [entered, setEntered] = (0, import_react.useState)(false), [origin, setOrigin] = (0, import_react.useState)({
		x: 0,
		y: 0,
		r: 0
	});
	const finish = (0, import_react.useRef)(null), clicked = (0, import_react.useRef)(false), music = (0, import_react.useRef)(null);
	(0, import_react.useEffect)(() => {
		if (entered) return;
		const stopMusic = startChiptune();
		const spin = setInterval(() => setFrame((i) => (i + 1) % 8), 125), load = setTimeout(() => setReady(true), 5e3);
		return () => {
			stopMusic();
			clearInterval(spin);
			clearTimeout(load);
	...
	function enter(event) {
		if (!ready || clicked.current) return;
		...
		setOpening(true);
		if (music.current) {
			music.current.a.close();
			clearInterval(music.current.t);
			music.current = null;
		}
		setTimeout(() => {
			try {
				...
				[
					...
					const o = a.createOscillator(), g = a.createGain(), t = a.currentTime + i * .11;
					o.type = "square";
					o.frequency.value = hz;
		...
		finish.current = setTimeout(() => setEntered(true), 1e3);
	}
	function playJingle() {
		try {
			const a = new AudioContext();
	...
	if (entered) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Page, {});
	const [x, width] = windows[frame];
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("main", {
		className: "loading-screen",
		"aria-label": "Загрузка игры",
		"aria-busy": !ready,
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)(StarTunnel, {}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "loading-center",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "fish-stage",
					children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(DancingFish, {})
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "loading-control",
					children: ready ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
						className: "enter-brain",
						"aria-label": "Войти в игру",
						disabled: opening,
						onClick: (event) => {
							playJingle();
							enter(event);
						...
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "brain-icon level-2",
							"aria-hidden": "true"
						})
					}) : /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "fish-loading",
						role: "status",
						"aria-label": "Загрузка",
						children: [
...
//#endregion
//#region client.tsx
(0, import_client.createRoot)(document.getElementById("root")).render(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(LoadingScreen, {}));
//#endregion
